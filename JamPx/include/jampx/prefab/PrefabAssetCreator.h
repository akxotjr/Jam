...
namespace jam::px::prefab
{
	...
	public:
		...
		static PxTriangleMesh*	CreateTriangleMesh(const string& path);
		static PxConvexMesh*	CreateConvexMesh(const string& path);
		...
		static PxRigidActor*	CreateRigidActor(const PrefabTemplateDef& def, const vector<PxShape*>& shapes);
	};
}
