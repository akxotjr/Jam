...
namespace jam::px
{
	...
	DefaultQuakePolicy::DefaultQuakePolicy(const MovementConfig& cfg)
		: m_cfg(cfg)
	{
	}

	WishMovement DefaultQuakePolicy::BuildWishMovement(const MoveIntent& intent) const
	{
	...
	void DefaultQuakePolicy::Integrate(MovementState& st, const WishMovement& wish, float dt) const
	{
        if (dt <= 0.0f) return;
