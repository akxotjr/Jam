...
namespace jam::net
{
	...
	{
		SINGLE,			// 1명
		MULTICAST,		// 그룹대상 전체에게 동일 payload
		FAN_OUT,		// 그룹대상 전체에게 per-target payload
	...
	struct TransportInfo
	{
		...
		using PayloadFactory = std::function<shared_ptr<SendBuffer>(uint64)>;
	...
	public:
		...
		virtual void Send(const TransportInfo& info, const shared_ptr<SendBuffer>& buf) = 0;
		virtual void EnumerateGroupUsers(uint32 groupId, const std::function<void(uint64)>& fn) = 0;
	};
}
